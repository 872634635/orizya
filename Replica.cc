	...
	void Replica::register_nondet_choices(void(*n) (Seqno, Byz_buffer *), 
		...
		void Replica::handle(Request * m)
			...
			if (has_new_view() && m->verify())
				...
				printf("end handle request msg\n");
			}

		void Replica::send_order_request(bool retransmitted)
			{
			printf("Begin send_order_request\n");
...
{
	printf("begin handle order_request\n");
	Seqno ms = m->seqno();
	int bsize = m->bsize();
	...
	if (in_wv(m) && ms > low_bound && has_new_view())
	{
		...
		if (oc.add(m))//将m加入oc该certificate中，如果m满足一定的条件
		{
			...
			else //if the replica is not the primary
			{
	...
	printf("end handle order request\n");
	return;
}
		...
		void Replica::execute_request_speculatively(Seqno s)
			...
			printf("end execute request msg\n");
			}
		...
		void Replica::execute_request(Seqno s, Request * req)
			{
			printf("begin real execute request msg\n");
			Digest			rh_d;
			...
			if (rh_log.comp_history_digest(s, &rh_d))
				...
				printf("end real execute request msg\n");
			}
		...
		{
			printf("begin send_mycommitted_locally\n");
			...
			printf("end send mylocalcommitted msg\n");
		}
		
		//根据上下文 还是改成handle(Mycommit*)
		void Replica::handle(Mycommit * m)
		{
			printf("begin handle mycommit msg\n");
			Seqno			ms	= m->seqno();
			int 			bsize = m->bsize();
			...
			if ( in_wv(m)&& ms > low_bound && m->verify() )  //&& has_new_view()
			{
				...
				if (ms > mcl)
				{
					...
					//   rep->request_id(),rep->id(),r_reps->num_correct(), r_reps->num_elements() );
						...
						printf("myc_reps add successful\n");
						
							//fprintf(stderr,"Added response with rid :%llu cid : %d Count : %d recvd : %d\n", 
							//rep->request_id(),rep->id(),r_reps->num_correct(), r_reps->num_elements());
					}
					
					printf("myc_reps:  num_correct= %d\n",myc_reps->num_correct());
					if (myc_reps->is_complete() ) //2f+1
					{
						printf("myc_reps->is_complete()\n");
						Digest			d;
				...
				{
					printf("not pass the mycommit msg\n");
					delete m;
				}
			printf("end handle mycommit msg\n");		
		}
			
		/////
		void Replica::send_mycommit(Order_request * m, Digest & d)
		{
			printf("begin send mycommit msg\n");
			Seqno ms = m->seqno();
			int bsize = m->bsize();
			View 	v = m->view();
			int Id = id();
